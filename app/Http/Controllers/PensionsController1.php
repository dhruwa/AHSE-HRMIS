...
class PensionsController extends Controller
{
    ...
    private function getRetiredEmployees($where = array(), $paginate = 50) {
        ...
        $employees = DB::table('employees')
            ...
            ->select('employees.emp_id as employeeId', 'employees.emp_f_name as emp_f_name', 'employees.emp_m_name as emp_m_name', 'employees.emp_l_name as emp_l_name', 'employees.emp_dob as dob', 'employees.emp_gender as gender', 'employees.emp_date_of_joining as dateOfJoining', 'employees.emp_gender as gender', 'employees.emp_contact_no as contactNumber', 'employees.emp_contact_no as contactNumber', 'employees.emp_alt_contact_no as altContactNumber', 'departments.fld_Department as departmentName', 'posts.fld_PostName as postName')
            ->paginate($paginate);
        foreach($employees as $k => $v) {
            $employees[$k]->date_of_retirement = '';
            $dob = new DateTime($v->dob);
            $retirement_date =  $dob->modify('+60 year');
            if(date('Y-m-d') < $retirement_date->format('Y-m-d') ) {
               unset($employees[$k]); 
            }else{
               $employees[$k]->date_of_retirement = $retirement_date->format('Y-m-d');
            }
        }
